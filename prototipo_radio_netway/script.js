...
function broadcastState(status) {
    try {
        ...
        var state = {
            status:    status || (radioActive ? 'playing' : 'paused'), // 'playing' | 'paused' | 'stopped' | 'ad'
            ...
            ts:        Date.now()   // força o evento storage mesmo se o resto não mudou
        };
...
function handleAdFileUpload(event) {
    // Aceitar tanto evento nativo quanto input direto
    var input = (event && event.target) ? event.target : event;
    var file = input.files ? input.files[0] : null;
    if (!file) return;

    // Aceitar qualquer audio (mp3, ogg, wav, etc)
    if (!file.type.match(/audio\//) && !file.name.match(/\.(mp3|ogg|wav|aac|m4a)$/i)) {
        showToast('Apenas arquivos de áudio são aceitos (.mp3, .wav, etc).');
        return;
    }
    if (file.size > 20 * 1024 * 1024) {
        showToast('Arquivo muito grande (máx. 20MB).');
        return;
    }
    ...
    if (progLabel) { progLabel.style.display = 'block'; progLabel.textContent = 'Lendo arquivo...'; }
    ...
    reader.onload = function(e) {
        ...
        if (progLabel) progLabel.textContent = 'Salvando...';
        ...
        if (progLabel) progLabel.textContent = 'Salvo!';
        setTimeout(function() {
...
function playVoiceAd() {
    ...
    // Guardar volume atual do player YT e abaixar
    var volBefore = currentVolume;
    var duckVol   = Math.max(Math.round(volBefore * 0.30), 5);

    if (player && playerReady) {
        try { player.setVolume(duckVol); } catch(e) {}
    }
    ...
    audio.play().catch(function(e) {
        console.warn('playVoiceAd: autoplay bloqueado', e);
        showToast('Clique na página para habilitar áudio.');
        restoreAfterAd(volBefore, ad);
    });

    audio.onended  = function() { restoreAfterAd(volBefore, ad); };
    audio.onerror  = function() {
        showToast('Erro ao tocar anúncio.');
        restoreAfterAd(volBefore, ad);
    };
}

function restoreAfterAd(originalVol, ad) {
    adCurrentAudio = null;

    // Restaurar volume
    if (player && playerReady) {
        try { player.setVolume(originalVol); } catch(e) {}
    }

    // Esconder banner e overlay
    var banner = document.getElementById('adPlayingBanner');
