...
function loadSiteData() {
    try {
        // Prioridade 1: dados do admin (cms.js salva em 'rnw_cms_v4')
        var cmsRaw = localStorage.getItem('rnw_cms_v4');
        if (cmsRaw) {
            var cms = JSON.parse(cmsRaw);
            if (cms.header) {
                siteData.companyName = cms.header.companyName || siteData.companyName;
                siteData.slogan      = cms.header.slogan      || siteData.slogan;
                siteData.whatsapp    = cms.header.whatsapp    || siteData.whatsapp;
            }
            if (Array.isArray(cms.banners) && cms.banners.length > 0) {
                siteData.banners = cms.banners;
            }
            return;
        }
        // Prioridade 2: fallback legado
...
// Timestamp da ultima atualizacao de CMS detectada pelo polling
var pubLastCmsUpdate = 0;

function pubSyncLoop() {
    // Sync do player de radio (broadcast)
    var state = pubReadBroadcast();
    if (state && state.ts !== pubLastBcastTs) {
        pubLastBcastTs = state.ts;
        pubApplyBroadcast(state);
    }

    // Sync dos banners/cms — detecta quando admin salvou (mesma aba ou outra)
    try {
        var cmsFlag = localStorage.getItem('rnw_cms_updated');
        if (cmsFlag) {
            var flagTs = parseInt(cmsFlag, 10) || 0;
            if (flagTs !== pubLastCmsUpdate) {
                pubLastCmsUpdate = flagTs;
                var cmsRaw = localStorage.getItem('rnw_cms_v4');
                if (cmsRaw) {
                    var cms = JSON.parse(cmsRaw);
                    if (cms.header) {
...
function pubStartSync() {
    ...
    window.addEventListener('storage', function(e) {
        ...
        // Sincronizar banners/dados quando admin salvar
        if (e.key === 'rnw_cms_v4') {
            try {
                var cms = e.newValue ? JSON.parse(e.newValue) : null;
                if (cms) {
                    if (cms.header) {
                        siteData.companyName = cms.header.companyName || siteData.companyName;
                        siteData.slogan      = cms.header.slogan      || siteData.slogan;
